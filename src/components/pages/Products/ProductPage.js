...
class ProductPage extends React.Component {
  ...
  componentDidMount() {
    ...
    // Load static files
    this.setState({placeholderImage: require('../../common/images/image_placeholder.png')});
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (this.state.addingToCart && this.state.cartLoading && !nextProps._cartLoading) {
      this.setState({addingToCart: false, quantity: 1});
  ...
  render() {
    ...
    let breadcrumbs = [
      {
        name: <FormattedMessage message={intlStore.getMessage(intlData, 'homepage')} locales={intlStore.getCurrentLocale()}/>,
        to: 'homepage',
        params: routeParams
      }, {
        name: <FormattedMessage message={intlStore.getMessage(intlData, 'productsList')} locales={intlStore.getCurrentLocale()}/>,
        to: 'products',
        params: routeParams
      }
    ];
    ...
    let addCollectionToBreadcrumbs = (collectionId) => {
      ...
      if (collection) {
        breadcrumbs.push({
          name: <FormattedMessage message={intlStore.getMessage(collection.name)} locales={intlStore.getCurrentLocale()}/>,
          to: 'collection-slug',
          ...
          : <div>
              <div className="product-page__header">
                <Breadcrumbs links={breadcrumbs} weight="bold">
                  <FormattedMessage message={intlStore.getMessage(this.state.product.name)} locales={intlStore.getCurrentLocale()}/>
                </Breadcrumbs>
              </div>

              <div className="product-page__product" itemScope="itemScope" itemType="http://schema.org/Product">
                ...
                <div className="product-page__details">
                  ...
                  </div>
                  {
                    this.state.product.pricing
                      ? <div className="product-page__price" itemProp="offers" itemScope="itemScope" itemType="http://schema.org/Offer">
                          <div style={{
                              display: 'none'
                            }} itemProp="price">
                          ...
                          <div>
                            <Text size="medium" weight="bold">
                              <FormattedNumber value={this.state.product.pricing.retail} style="currency" currency={this.state.product.pricing.currency}/>
                            </Text>
                          </div>
                        </div>
                      : null
                  ...
                  <div className="product-page__add">
                    <div className="product-page__quantity">
                      <QuantitySelector value={this.state.quantity} onChange={this.handleQuantityChange}/>
                    </div>
                    {/*<div className="product-page__add-buttons">
                                        {this.state.product.stock > 0 ?
                                          <Link to="customize" params={routeParams} product={this.state.product}>
                                              <Button type="primary"
                                                      onClick={this.handleAddPrescriptionClick}
                    ...
                    <div className="product-page__add-buttons">
                      {
                        this.state.product.stock > 0
                          ? <Button type="primary" onClick={this.handleOrderClick} disabled={this.state.quantity <= 0 || this.state.cartLoading}>
                              Order
                            </Button>
                          : <Button type="primary" disabled={true}>
                              <FormattedMessage message={intlStore.getMessage(intlData, 'outOfStock')} locales={intlStore.getCurrentLocale()}/>
                            </Button>
                      }
                    </div>
                  ...
                  <div className="product-page__description">
                    <div className="product-page__description-label">
                      <Heading size="medium">
                        <FormattedMessage message={intlStore.getMessage(intlData, 'descriptionLabel')} locales={intlStore.getCurrentLocale()}/>
                      </Heading>
                    </div>
                    <div className="product-page__description-content" itemProp="description">
                      <Text size="small">
                        <FormattedMessage message={intlStore.getMessage(this.state.product.description)} locales={intlStore.getCurrentLocale()}/>
                      </Text>
                    </div>
                  </div>

                  {
                    this.state.contents.map(function(content) {
                      return (<div className="product-page__content">
                  ...
                  !this.state.suggestionsLoading && this.state.suggestions.length === 0
                    ...
                    : <div className="product-page__suggestions">
                        <ProductSuggestions products={this.state.suggestions} loading={this.state.suggestionsLoading}>
                          <FormattedMessage message={intlStore.getMessage(intlData, 'crossSell')} locales={intlStore.getCurrentLocale()}/>
                        </ProductSuggestions>
                      </div>
                }
              </div>
              ...
              <div>
                {
                  this.state.customizing &&
                     <div>
                        <PrescriptionModal prescription={this.state.prescription} handleFieldChange={this.handlePrescriptionFieldChange.bind(this)}></PrescriptionModal>

                        <div className="product-page__add-buttons">
                          {
                            this.state.product.stock > 0
                              ? <Button type="primary" onClick={this.handleAddToCartClick} disabled={this.state.quantity <= 0 || this.state.cartLoading}>
                                  <FormattedMessage message={intlStore.getMessage(intlData, 'addToCart')} locales={intlStore.getCurrentLocale()}/>
                                </Button>
                              : <Button type="primary" disabled={true}>
                                  <FormattedMessage message={intlStore.getMessage(intlData, 'outOfStock')} locales={intlStore.getCurrentLocale()}/>
                                </Button>
 ...
 * Flux
 */
ProductPage = connectToStores(ProductPage, [
  CartStore, ProductContentsStore, ProductDetailsStore, ProductSuggestionsStore
], (context) => {
  return {
